"""
finetune_osnet_veri776.py

Fine-tune OSNet on VeRi-776 for vehicle re-identification, with auxiliary
color and type classification heads to shape a stronger backbone.

Usage:
    python finetune_osnet_veri776.py --data-root /path/to/VeRi

Notes for the eventual INT8 / Jetson Orin Nano deployment path:
  * The color and type auxiliary heads exist purely to shape a better
    backbone during training; the deployment checkpoint contains the bare
    feature extractor only.
  * `osnet_x0_75` or `osnet_x0_5` for tighter latency budgets.
  * Train at the resolution you intend to deploy at; static shapes are kinder
    to TensorRT INT8 calibration.
  ...
  * No IBN / GN / IN -- only ReLU + BatchNorm + standard convs, all of which
    map to native INT8 ops in TensorRT.
"""
...
# ---------------------------------------------------------------------------
# Model wrapper with auxiliary heads
# ---------------------------------------------------------------------------


class OSNetWithAuxHeads(nn.Module):
    """
    Wraps a torchreid OSNet (built with loss='triplet') and adds two
    auxiliary classifiers on the embedding for vehicle color and type.

    Train mode -> (id_logits, features, color_logits, type_logits)
    Eval  mode -> features only (matches the bare backbone, so eval code
                  is unchanged)
    """

    def __init__(self, backbone, num_colors=NUM_COLORS, num_types=NUM_TYPES):
        super().__init__()
        self.backbone = backbone
        feat_dim = backbone.feature_dim  # torchreid OSNet exposes this
        self.color_head = nn.Linear(feat_dim, num_colors)
        self.type_head = nn.Linear(feat_dim, num_types)

    def forward(self, x):
        if self.training:
            id_logits, features = self.backbone(x)
            color_logits = self.color_head(features)
            type_logits = self.type_head(features)
            return id_logits, features, color_logits, type_logits
...
def main():
    ...
    # OSNet's reference recipe (paper + torchreid configs) is 250 epochs from
    # ImageNet pretrained. 120 is a good time/quality compromise; 60 is
    # undertrained for this architecture.
    ...
    model = OSNetWithAuxHeads(backbone).to(device)
    ...
    best_path = save_dir / "osnet_veri776_best.pt"
    best_map = 0.0
    ...
    # INT8 calibration later -- the aux heads are training-time scaffolding
    # that doesn't ship.
    if best_path.is_file():
        ...
        deploy_path = save_dir / "osnet_veri776_deploy.pt"
        torch.save(
            {
