...
def _file_results_iter(path, detector, vid_stride=1):
    """File -> per-frame detector results, with Ultralytics handling I/O.

    `vid_stride` skips frames in the source loader (1 = every frame,
    5 = every 5th, etc.). Useful for fast-forwarding through long files.
    """
...
def run_pipeline(video_source, use_gstreamer=True, vid_stride=1):
    ...
    try:
        for results in results_iter:
            now = time.time()
            fps_buf.append(now - last_iter_time)
            last_iter_time = now
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stride", type=int, default=1,
                        help="Process every Nth frame from a file source. "
                             "Higher = faster playback (file mode only).")
    parser.add_argument("--flip", type=int, default=0,
                        help="GStreamer flip-method (0-3) for the CSI camera.")
    args = parser.parse_args()

    if args.source:
        print(f"Reading from file: {args.source} (stride={args.stride})\n")
        run_pipeline(args.source, use_gstreamer=False, vid_stride=args.stride)
    else:
